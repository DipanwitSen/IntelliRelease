 ...
 * Inline SVG icon set.
 *
 * Bundled rather than fetched: the CSP on this app forbids external hosts, and
 * an icon font would ship far more glyphs than the ~60 used here. Paths use
 * `currentColor` and a 24×24 viewBox, so an icon inherits colour from its
 * parent and scales with the `size` input alone.
 *
 * The markup below is a compile-time constant with no interpolation, which is
 * why bypassing the sanitizer is safe — `name` only ever selects a key, it is
 * never rendered.
 */
const ICONS: Readonly<Record<string, string>> = {
  ...
  'git-pull-request': '<circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><path d="M6 9v6"/><circle cx="18" cy="18" r="3"/><path d="M18 15V9a4 4 0 0 0-4-4h-3"/><path d="m14 8-3-3 3-3"/>',
  ...
  sparkles: '<path d="M9.94 5.33 12 2l2.06 3.33 3.69.77-2.5 2.85.42 3.75L12 11.2l-3.67 1.5.42-3.75-2.5-2.85z"/><path d="M5 15l.9 2.1L8 18l-2.1.9L5 21l-.9-2.1L2 18l2.1-.9z"/><path d="M18 14l.7 1.6 1.6.7-1.6.7-.7 1.6-.7-1.6-1.6-.7 1.6-.7z"/>',
  network: '<rect x="9" y="2" width="6" height="5" rx="1"/><rect x="2" y="17" width="6" height="5" rx="1"/><rect x="16" y="17" width="6" height="5" rx="1"/><path d="M12 7v5"/><path d="M5 17v-2a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v2"/>',
  ...
  workflow: '<rect x="3" y="3" width="7" height="6" rx="1"/><rect x="14" y="15" width="7" height="6" rx="1"/><path d="M6.5 9v4a2 2 0 0 0 2 2h5.5"/>',
  plug: '<path d="M12 22v-5"/><path d="M9 8V2"/><path d="M15 8V2"/><path d="M18 8v4a6 6 0 0 1-12 0V8z"/>',
  'alert-triangle': '<path d="m10.29 3.86-8.19 14A2 2 0 0 0 3.81 21h16.38a2 2 0 0 0 1.71-3.14l-8.19-14a2 2 0 0 0-3.42 0z"/><path d="M12 9v4"/><path d="M12 17h.01"/>',
  ...
  cloud: '<path d="M17.5 19a4.5 4.5 0 0 0 .5-8.97A6 6 0 0 0 6.3 9.4 4 4 0 0 0 7 19z"/>',
  code: '<path d="m16 18 6-6-6-6"/><path d="m8 6-6 6 6 6"/>',
  'file-code': '<path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7z"/><path d="M14 2v5h6"/><path d="m10 12-2 2 2 2"/><path d="m14 12 2 2-2 2"/>',
  layers: '<path d="m12 2 9 5-9 5-9-5z"/><path d="m3 12 9 5 9-5"/><path d="m3 17 9 5 9-5"/>',
  box: '<path d="m21 8-9-5-9 5v8l9 5 9-5z"/><path d="m3 8 9 5 9-5"/><path d="M12 13v9"/>',
  ...
  'file-search': '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h5"/><path d="M14 2v5h6"/><circle cx="17" cy="16" r="3"/><path d="m21 20-1.9-1.9"/>',
...
export class IconComponent {
  ...
  protected readonly markup = computed<SafeHtml>(() => {
    // Unknown names render an empty box rather than throwing, so a typo in a
    // data-driven icon key degrades to a blank space instead of a broken page.
    const body = ICONS[this.name()] ?? '';
    return this.sanitizer.bypassSecurityTrustHtml(body);
  });
}

/** Exposed so data-driven icon keys can be validated in tests. */
