...
- Frontend: Angular 18.2, TypeScript 5.5, Node.js 20+
- Backend: Java 21, Spring Boot 3.5, Maven (wrapper committed — no separate Maven install)
- AI Service: Python 3.12, FastAPI, Pydantic
- Database: PostgreSQL 17 (primary, Docker Compose), H2 in-memory (`dev` profile only)
- LLM provider: Groq hosted API (primary), Ollama (local fallback), deterministic
  templates (final fallback — the app never hard-fails on model unavailability)
- Email: MailHog (POC SMTP trap), optional Office365 SMTP relay mirror
- API Docs: Swagger/OpenAPI (springdoc-openapi)
- Container runtime: Docker Compose (POC only)
...
11. Raw source code is never sent to the model (`LLM_SEND_DIFFS` defaults false)
12. Flyway is the single source of truth for schema — Hibernate never writes DDL
...
│   ├── mvnw, mvnw.cmd # Maven wrapper — the supported way to build
│   └── src/main/
│       ├── java/com/gyansys/intellirelease/
│       │   ├── api/         # REST controllers
│       │   ├── application/ # Service orchestration
│       │   ├── domain/      # Business logic (context, impact, risk, regression,
│       │   │                #   drift, cleanup, readiness, release)
│       │   ├── adapters/    # External integrations (GitHub, Jira, AI service,
│       │   │                #   notifications)
│       │   ├── config/      # Security, mail, CORS, OpenAPI wiring
│       │   └── infra/       # Database, job queue, audit, tenant context
│       └── resources/
│           ├── application.yml, application-dev.yml
│           └── db/migration/{h2,postgresql}/  # Flyway SQL, one set per vendor
├── ai-service/        # Python 3.12 / FastAPI
│   └── app/           # main, service, providers, prompts, schemas, validation
├── docs/
└── docker-compose.yml # postgres + mailhog + ai + backend + frontend
```

## Database profiles
Two engines, one schema shape. Flyway resolves `{vendor}` to pick the matching
migration set; only JSONB/UUID-default syntax differs between them.

| Mode | How it runs | Datasource | Persistence |
|---|---|---|---|
| `dev` profile | `mvnw spring-boot:run -Dspring-boot.run.profiles=dev` | H2 in-memory | Wiped every restart |
| Docker Compose | `docker compose up` | PostgreSQL 17 | Persisted in the `pgdata` volume |

PostgreSQL is selected purely by environment variable (`DATABASE_URL`, `DB_DRIVER`,
`DB_DIALECT`, `FLYWAY_LOCATIONS`) — there is no Postgres-specific Spring profile.
